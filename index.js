...
if (TOKENS.length === 0) {
  console.error(
    "[FATAL] DISCORD_TOKEN not found in .env or environment variables (use comma-separated tokens for multiple users)",
  );
  process.exit(1);
}
...
const rawPresence = (
  env.DISCORD_PRESENCE ||
  process.env.DISCORD_PRESENCE ||
  "dnd"
)
  .toLowerCase()
  .trim();
const PRESENCE_STATUS = VALID_PRESENCE_STATUSES.has(rawPresence)
  ? rawPresence
  : "dnd";
if (rawPresence && !VALID_PRESENCE_STATUSES.has(rawPresence)) {
  console.warn(
    `[quests] Invalid DISCORD_PRESENCE "${rawPresence}", using ${PRESENCE_STATUS}`,
  );
}
...
const SUPER_PROPS = {
  ...
  client_build_number: 522860,
  client_event_source: null,
  client_launch_id: "d6bcd3b7-a615-4e98-8560-5eddd86a5b0e",
  launch_signature: "82748bc6-22d5-466d-9c74-d9aa40afb694",
  client_heartbeat_session_id: "c0231b23-ee5d-41a7-b7b7-aff28aa22bd9",
  client_app_state: "focused",
};
const SUPER_PROPS_B64 = Buffer.from(JSON.stringify(SUPER_PROPS)).toString(
  "base64",
);
const rawCheckMinutes =
  env.CHECK_INTERVAL_MINUTES ?? process.env.CHECK_INTERVAL_MINUTES;
const parsedCheckMinutes =
  rawCheckMinutes != null && String(rawCheckMinutes).trim() !== ""
    ? parseFloat(String(rawCheckMinutes).trim())
    : NaN;
const CHECK_INTERVAL_MS =
  !Number.isNaN(parsedCheckMinutes) && parsedCheckMinutes > 0
    ? Math.max(1000, Math.round(parsedCheckMinutes * 60 * 1000))
    : 5 * 60 * 1000;
const WEBHOOK_NEW =
  (
    env.QUEST_WEBHOOK_NEW ||
    process.env.QUEST_WEBHOOK_NEW ||
    env.QUEST_WEBHOOK_URL ||
    process.env.QUEST_WEBHOOK_URL ||
    ""
  ).trim() || null;
const WEBHOOK_COMPLETED =
  (
    env.QUEST_WEBHOOK_COMPLETED ||
    process.env.QUEST_WEBHOOK_COMPLETED ||
    env.QUEST_WEBHOOK_URL ||
    process.env.QUEST_WEBHOOK_URL ||
    ""
  ).trim() || null;
const WEBHOOK_GATEWAY =
  (
    env.QUEST_WEBHOOK_GATEWAY ||
    process.env.QUEST_WEBHOOK_GATEWAY ||
    env.QUEST_WEBHOOK_URL ||
    process.env.QUEST_WEBHOOK_URL ||
    ""
  ).trim() || null;
...
function notifyNewQuest(user, quest) {
  ...
  postWebhook(
    WEBHOOK_NEW,
    webhookBodyWithUserPing(
      user,
      `**New quest** — ${name} · ${accountLabel(user)}`,
      {
        event: "new_quest",
...
function notifyQuestCompleted(user, quest) {
  ...
  postWebhook(
    ...
    webhookBodyWithUserPing(
      user,
      `**Quest completed** — ${name} · ${accountLabel(user)}`,
      {
...
function notifyGatewayConnected(user, snap) {
  ...
  postWebhook(
    WEBHOOK_GATEWAY,
    webhookBodyWithUserPing(user, line, {
  ...
  function connectGateway(userLog) {
    return new Promise((resolve, reject) => {
      ...
      const setPresence = (activities) => {
        send({
          op: 3,
          d: { since: null, activities, status: PRESENCE_STATUS, afk: false },
        });
      };
        ...
        if (msg.t === "READY") {
          ...
          (async () => {
            ...
            try {
              ...
              userLog(
                "WARN",
                `Could not load quest / claim status: ${e.message}`,
              );
            }
            if (snap) {
              userLog(
                "INFO",
                `Claim status: ${c.bold}${snap.leftToClaim}${c.reset} left to claim ${c.dim}(${snap.active.length} active · ${snap.pending.length} pending · ${snap.completed.length} completed)${c.reset}`,
              );
            }
      ...
      ws.on("close", (code) => {
        ...
        userLog(
          "WARN",
          `Gateway disconnected ${c.dim}(code: ${code})${c.reset} — reconnecting in ${RECONNECT_DELAY_MS / 1000}s`,
        );
        setTimeout(() => {
        ...
        res.on("end", () => {
          try {
            ...
            if (res.statusCode >= 400) {
              reject(
                new Error(
                  `HTTP ${res.statusCode}: ${json?.message || json?.error || data}`,
                ),
              );
            } else {
              resolve(json);
...
function getTaskInfo(quest) {
  const tasks =
    quest.config?.task_config_v2?.tasks || quest.config?.task_config?.tasks;
  if (!tasks) return { type: null, required: 900 };
  const first = Object.values(tasks)[0];
  return {
    type: first.type || first.event_name,
    required: first.target || 900,
  };
}
...
async function completeQuest(quest, setPresence, api, userLog, user) {
  ...
  userLog(
    "QUEST",
    `${c.bold}${c.cyan}${name}${c.reset} ${c.dim}(${type})${c.reset}`,
  );
    ...
    return new Promise((resolve) => {
      const sendHB = async () => {
        try {
          ...
          userLog(
            "INFO",
            `${c.magenta}[${name}]${c.reset} ${formatProgress(progress, required)}`,
          );
          ...
          userLog(
            "ERROR",
            `${c.red}[${name}]${c.reset} Heartbeat failed: ${e.message}`,
          );
          return false;
        }
      };
  ...
  if (type === "ACHIEVEMENT_IN_ACTIVITY") {
    ...
    const devAppId =
      taskConfig?.developer_application_id || task?.applications?.[0]?.id;
    const appId = quest.config?.application?.id;
    const targetAppId = devAppId || appId;

    userLog(
      "INFO",
      `Achievement in activity ${c.dim}(app: ${targetAppId})${c.reset}`,
    );
    userLog("INFO", `Getting OAuth2 auth code...`);

    let authCode = null;
    try {
      const oauthRes = await api(
        "POST",
    ...
    try {
      const authRes = await requestJSON(
        "POST",
        `https://${targetAppId}.discordsays.com/.proxy/acf/authorize`,
        { code: authCode },
      );
    ...
    userLog(
      "INFO",
      `Sending achievement progress ${c.dim}(target: ${required})${c.reset}`,
    );

    try {
      const progressRes = await requestJSON(
        "POST",
        `https://${targetAppId}.discordsays.com/.proxy/acf/quest/progress`,
        { progress: required },
        {
    ...
    try {
      ...
      if (updated && isCompleted(updated)) {
        ...
        userLog(
          "INFO",
          `Progress sent — ${c.dim}check Discord for status${c.reset}`,
        );
      }
    } catch {}
    return;
  ...
  if (type === "WATCH_VIDEO" || type === "WATCH_VIDEO_ON_MOBILE") {
    ...
    return new Promise((resolve) => {
      const iv = setInterval(async () => {
        ...
        try {
          await api("POST", `/quests/${questId}/video-progress`, {
            timestamp: nextProgress,
          });
          ...
          userLog(
            "WARN",
            `${c.red}[${name}]${c.reset} Video progress request failed: ${e.message} ${c.dim}(will retry)${c.reset}`,
          );
        }
      }, VIDEO_TICK_MS);
    });
  ...
  if (
    ["PLAY_ON_DESKTOP", "PLAY_ON_DESKTOP_V2", "STREAM_ON_DESKTOP"].includes(
      type,
    )
  ) {
    const appId = quest.config?.application?.id;
    ...
    userLog(
      "INFO",
      `Simulating ${c.bold}${appName}${c.reset} ${c.dim}(${appId})${c.reset}`,
    );
    setPresence([
      {
        name: appName,
    ...
    userLog(
      "INFO",
      `${c.dim}Heartbeats every ${HEARTBEAT_INTERVAL_MS / 1000}s${c.reset}`,
    );
...
async function checkAndComplete(
  ...
  try {
    const { active, pending, completed, leftToClaim } =
      await fetchActiveQuestSnapshot(api);
    ...
    userLog(
      "INFO",
      `Found ${c.bold}${active.length}${c.reset} quest(s): ${c.yellow}${pending.length} pending${c.reset}, ${c.green}${completed.length} completed${c.reset}, ${c.magenta}${leftToClaim} left to claim${c.reset}`,
    );

    if (pending.length === 0) {
      userLog(
        "OK",
        `All quests completed ${c.dim}— ${c.bold}${leftToClaim}${c.reset}${c.dim} reward(s) left to claim in Discord${c.reset}`,
      );
      return;
    }
...
async function runUserSession(token, index) {
  ...
  userLog(
    "INFO",
    `${c.dim}Check interval: ${CHECK_INTERVAL_MS / 60_000}min${c.reset}`,
  );

  const questPollState = { lastActiveIds: null };
  await checkAndComplete(setPresence, api, userLog, user, questPollState);
  setInterval(
    () => checkAndComplete(setPresence, api, userLog, user, questPollState),
    CHECK_INTERVAL_MS,
  );
}
...
async function main() {
  ...
  log(
    "INFO",
    `${c.bold}${TOKENS.length}${c.reset} account(s) — ${c.dim}DISCORD_TOKEN${c.reset} comma-separated`,
  );
  log(
    "INFO",
    `Presence: ${c.bold}${PRESENCE_STATUS}${c.reset} ${c.dim}(DISCORD_PRESENCE: online | idle | dnd | invisible)${c.reset}`,
  );
